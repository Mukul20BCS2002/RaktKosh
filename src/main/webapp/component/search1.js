...
// Set up an object that maps each state to its districts
const districtMap = {
	"Andhra Pradesh": ["Anantapur", "Chittoor", "East Godai", "Guntur", "Krishna", "Kurnool", "Prakasam", "Srikakulam", "Sri Potti Sriramulu Nellore", "Visakhapatnam", "Vizianagaram", "West Godai", "YSR District, Kadapa"],
...
stateSelect.addEventListener('change', () => {
	...
	// Clear any existing options from the district dropdown
	districtSelect.innerHTML = '<option value="">--Select District--</option>';

	// Add options to the district dropdown based on the selected state
	for (let i = 0;i < districtMap[selectedState].length; i++) {
		const option = document.createElement('option');
		option.value = districtMap[selectedState][i];
		option.textContent = districtMap[selectedState][i];
